...
#A=$(find linux-5.11.13/ -type f -size 1k -perm -u=w | wc -l)
B=$(find linux-5.11.13/ -type f -size -1024c -perm -u=w | wc -l) #zaokroglenia w ramach jednostki
echo $B

#-writable alternatywnie nie, ale -not perm inni grupa
#czy wynik moze zalezec od moich ustawien dostepow? nie
#czy w poleceniu nie powinno byc <= nie
