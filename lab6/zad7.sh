...
awk '{
	for (i = 1; i <= NF; i++) { 
		if ($i ~ /.*[@].*/ || $i ~ /[0-9]*[[:space:]]*[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]/) 
			printf $i "\n" 
	} 
}' dodatkowe/kant.txt

#grep -E -o '[[:space:]].*[@].*[[:space:]]' dodatkowe/kant.txt
#grep leci linijka po linijce czy slowo po slowie
#nr telefonu tez grep lepiej
