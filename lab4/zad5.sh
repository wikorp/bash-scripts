...
grep 'syslog*' dodatkowe/etc-services | rev | cut -d " " -f1 | rev | cut -d "/" -f1 | sort | uniq


#cut -d " " -f2- ucina lewa strone
#da sie uciac jakos od prawej strony zeby co chwile nie obracac, czy jest jakis domyslny separator
#w cut, skod odroznic czy spacja czy tab, program tr ma opcje -s " " wielokrotne powtorzenia zamienia na jedna spacje
